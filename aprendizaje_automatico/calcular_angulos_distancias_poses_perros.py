...
# LEER LOS DATOS DEL CSV DE POSES DE PERROS.
...
# CALCULAR EL ÁNGULO ENTRE TRES PUNTOS TENIENDO EN CUENTA LA ORIENTACIÓN DEL CUERPO.
...
# GENERAR LOS CÁLCULOS DE ÁNGULOS Y DISTANCIAS PARA CADA FRAME Y GUARDARLOS EN UN CSV.
def generar_calculos_csv():
    ...
    ruta_csv = os.path.join(os.path.dirname(os.path.abspath(__file__)), "angulos_distancias_poses_perros.csv")
    with open(ruta_csv, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(resultados)
    print("Resultados guardados como angulos_distancias_poses_perros.csv")

# EJECUTAR LA FUNCIÓN PRINCIPAL PARA GENERAR EL CSV CON LOS CÁLCULOS.
