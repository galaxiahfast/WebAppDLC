...
# LEER LOS DATOS DEL ARCHIVO CSV DE POSES DE PERROS.
def leer_csv(filename="poses_perros.csv"):
...
# OBTENER LA ETIQUETA DE CLASE SEGÚN SU NÚMERO.
...
# CALCULAR EL ÁNGULO ENTRE TRES PUNTOS CONSIDERANDO LA DIRECCIÓN DEL CUERPO.
...
# DIBUJAR LOS ARCOS DE LOS ÁNGULOS CALCULADOS EN LA IMAGEN.
def dibujar_arco(ax, punto_central, punto_inicio, punto_fin, angulo, radio=20, withers=None, tail_set=None):
    ...
    ax.text(text_x, text_y, f"{angulo:.2f}°", color='red', fontsize=10, fontweight='bold', fontfamily='Times New Roman', fontstyle='italic')

# AJUSTAR LOS LÍMITES DEL GRÁFICO PARA CENTRAR LA FIGURA.
...
# GRAFICAR LOS ESQUELETOS Y LOS CÁLCULOS EN CADA SUBPLOT.
def graficar_esqueleto(frame_data, ax, mostrar_calculos=False):
    bodyparts = ["Nose", "Withers", "Tail_Set", "Tail_Tip", "Right_Front_Elbow", "Right_Front_Wrist", "Right_Front_Paw", "Left_Front_Elbow", "Left_Front_Wrist", "Left_Front_Paw", "Right_Back_Elbow", "Right_Back_Wrist", "Right_Back_Paw", "Left_Back_Elbow", "Left_Back_Wrist", "Left_Back_Paw"]
    skeleton = [("Nose", "Withers"), ("Withers", "Tail_Set"), ("Tail_Set", "Tail_Tip"), ("Withers", "Right_Front_Elbow"), ("Withers", "Left_Front_Elbow"), ("Tail_Set", "Right_Back_Elbow"), ("Tail_Set", "Left_Back_Elbow"), ("Right_Front_Elbow", "Right_Front_Wrist"), ("Right_Front_Wrist", "Right_Front_Paw"), ("Left_Front_Elbow", "Left_Front_Wrist"), ("Left_Front_Wrist", "Left_Front_Paw"), ("Right_Back_Elbow", "Right_Back_Wrist"), ("Right_Back_Wrist", "Right_Back_Paw"), ("Left_Back_Elbow", "Left_Back_Wrist"), ("Left_Back_Wrist", "Left_Back_Paw")]
    coords = {}
    ...
    for i, part in enumerate(bodyparts):
        ...
        if confianza >= 0.75:
            ...
            ax.text(x + 10, y, part, fontsize=8, fontfamily='Times New Roman', fontstyle='italic', color='black')
    ...
    angulos = [("Withers", "Tail_Set", "Tail_Tip"), ("Tail_Set", "Nose", "Withers"), ("Right_Front_Elbow", "Right_Front_Wrist", "Right_Front_Paw"), ("Left_Front_Elbow", "Left_Front_Wrist", "Left_Front_Paw"), ("Right_Back_Paw", "Right_Back_Wrist", "Right_Back_Elbow"), ("Left_Back_Paw", "Left_Back_Wrist", "Left_Back_Elbow")]
    ...
    pares_distancia = [("Left_Front_Paw", "Left_Back_Paw"), ("Right_Front_Paw", "Right_Back_Paw")]
    for p1, p2 in pares_distancia:
        if p1 in coords and p2 in coords:
            ...
            mid_x = (x1 + x2) / 2
            mid_y = (y1 + y2) / 2
            ax.text(mid_x, mid_y, f"{distancia:.2f}", color='purple', fontsize=10, fontweight='bold', fontfamily='Times New Roman', fontstyle='italic')
    ajustar_limites(ax, coords)
    ax.set_xlabel("Coordenadas X", fontsize=12, fontfamily='Times New Roman', fontstyle='italic')
    ax.set_ylabel("Coordenadas Y", fontsize=12, fontfamily='Times New Roman', fontstyle='italic')
    ax.tick_params(axis='both', which='major', labelsize=10)
...
# GENERAR LAS IMÁGENES AGRUPADAS DE 2X5 POR CLASE Y CONDICIÓN.
def generar_imagenes_agrupadas():
    frames, clases = leer_csv()
    indices = np.arange(len(frames))
    np.random.shuffle(indices)
    frames = [frames[i] for i in indices]
    clases = [clases[i] for i in indices]
    carpeta_base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "img")
    carpeta_sin = os.path.join(carpeta_base, "poses_perros_sin_calculos")
    carpeta_con = os.path.join(carpeta_base, "poses_perros_con_calculos")
    os.makedirs(carpeta_sin, exist_ok=True)
    os.makedirs(carpeta_con, exist_ok=True)
    filas = 2
    columnas = 5
    imgs_por_grupo = filas * columnas
    for mostrar, carpeta in [(False, carpeta_sin), (True, carpeta_con)]:
        for grupo_idx in range(0, len(frames), imgs_por_grupo):
            fig, axs = plt.subplots(filas, columnas, figsize=(columnas*4, filas*4))
            axs = axs.flatten()
            for i in range(imgs_por_grupo):
                idx = grupo_idx + i
                if idx >= len(frames):
                    axs[i].axis('off')
                    continue
                ax = axs[i]
                graficar_esqueleto(frames[idx], ax, mostrar_calculos=mostrar)
                etiqueta = obtener_etiqueta(clases[idx])
                numero = f"{idx + 1:03d}"
                ax.set_title(f"Fotograma {numero} - {etiqueta}", fontsize=10, fontfamily='Times New Roman', fontstyle='italic')
                ax.invert_yaxis()
                ax.set_aspect('equal')
            plt.tight_layout()
            nombre = f"grupo_{grupo_idx // imgs_por_grupo + 1:03d}.png"
            ruta = os.path.join(carpeta, nombre)
            plt.savefig(ruta, bbox_inches='tight')
            plt.close(fig)
    total_imgs = ((len(frames) + imgs_por_grupo - 1) // imgs_por_grupo) * 2
    print(f"Se generaron {total_imgs} imágenes agrupadas en '{carpeta_sin}' y '{carpeta_con}'")

# LLAMAR A LA FUNCIÓN PRINCIPAL PARA GENERAR LAS IMÁGENES.
generar_imagenes_agrupadas()